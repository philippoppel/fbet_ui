...
vi.mock('@/app/api/lib/hash', () => ({
  hashPassword: vi.fn(),
  verifyPassword: vi.fn(),
}));
vi.mock('@/app/api/lib/jwt', () => ({
  signJwt: vi.fn(() => 'signed-token'),
}));

const { prisma } = await import('@/app/api/lib/prisma');
const { hashPassword, verifyPassword } = await import('@/app/api/lib/hash');
const { signJwt } = await import('@/app/api/lib/jwt');
const { createUser, authenticateUser } = await import('@/app/api/lib/userService');
